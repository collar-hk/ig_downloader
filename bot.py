...
Stories now use yt-dlp (cookies from Instaloader sessions) to avoid the
broken GraphQL query_hash.
"""
...
def _cookies_to_netscape(loader: instaloader.Instaloader, path: Path) -> None:
    """Write Instaloader session cookies to a Netscape cookies.txt file for yt-dlp."""
    lines = [
        "# Netscape HTTP Cookie File",
        "# This file was generated by the Telegram downloader bot",
        "",
    ]
    try:
        for cookie in loader.context._session.cookies:
            ...
            lines.append(
                f"{domain}\t{flag}\t{path_str}\t{secure}\t{expires}\t{cookie.name}\t{cookie.value}"
            )
    except Exception as exc:
        logger.warning("Could not extract cookies from session: %s", exc)
...
def download_story_sync(username: str, media_id: str | None, target_dir: str) -> None:
    """Download Instagram stories via yt-dlp using cookies from a rotated Instaloader session."""
    username = _normalize_username(username)
    session_files = get_available_session_files()
    attempts = max(1, len(session_files))
    ...
    for attempt in range(attempts):
        ...
        try:
            loader, current_user = get_thread_safe_loader(rotate_session=(attempt > 0))
            if not current_user:
                raise DownloadError("No valid Instagram session available for stories.")

            # Write temporary Netscape cookies from the loaded session
            cookie_path = Path(tempfile.mkstemp(prefix="ig_cookies_", suffix=".txt")[1])
            _cookies_to_netscape(loader, cookie_path)
            ...
            ydl_opts = {
                ...
                "noplaylist": False,  # allow all active stories when no media_id
            }
            ...
            last_exc = exc
            logger.warning(
                "Story attempt %s failed for @%s with session '%s': %s",
                attempt + 1,
                username,
                current_user if "current_user" in locals() else "?",
                exc,
            )
            if attempt == attempts - 1:
    ...
    if last_exc:
        raise DownloadError(f"Failed to fetch that story: {last_exc}") from last_exc
